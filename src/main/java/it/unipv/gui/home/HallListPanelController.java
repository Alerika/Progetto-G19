...
public class HallListPanelController implements ICloseablePane {
    ...
    private void initHallNameList() {
        hallNames = hallOperations.retrieveHallNames();
    ...
    private void initPreview() {
        ...
        for(int i = 0; i<hallNamesSize; i++) {
            previews.add(hallOperations.retrieveHallPreviewAsImage(hallNames.get(i), 220, 395, true, true));
        }
    }
    ...
    private void openHallPreview(String nomeSala) {
        ...
        try {
            fis = hallOperations.retrieveHallPreviewAsStream(nomeSala);
            image = new Image(fis);
        } finally {
    ...
    private List<Seat> initDraggableSeatsList(String nomeSala) { return hallOperations.retrieveSeats(nomeSala); }
